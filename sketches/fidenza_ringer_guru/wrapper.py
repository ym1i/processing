...
class Wrapper():
    ...
    def wrap(self, vertices, padding):
        for i, v in enumerate(vertices):
            next = vertices[(i + 1) % len(vertices)]
            a = PVector.sub(next, v).heading() - HALF_PI
            r = padding
            if i != 0 and abs(a - prev_a) < 0.3 : 
                self.vertices.append(PVector(v.x + r * cos(a), v.y + r * sin(a)))
            prev_a = a
    ...
    def render(self, c, sw=1, no_fill=False, no_stroke=False, curve=False):
        fill(c)
        stroke(0)
        strokeWeight(sw)
        if no_fill:
