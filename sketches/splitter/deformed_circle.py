...
class DeformedCircle():
    ...
    def set_vertices(self):
        ...
        for i in range(self.n_points):
            ...
            n = map(noise(x * resolution, y * resolution), 0, 1, -self.noise_scale, self.noise_scale)
    ...
    def double_circle(self, inner, outer, palette):
        stroke(0)
        strokeWeight(1)
        beginShape(QUAD_STRIP)
        for i in range(len(inner.vertices)):
            fill(palette.random_color())
            vertex(inner.vertices[i].x, inner.vertices[i].y)
            vertex(outer.vertices[i].x, outer.vertices[i].y)
        endShape(CLOSE) 
...
def double_circle(inner, outer, palette):
        stroke(0)
        strokeWeight(1)
        beginShape(QUAD_STRIP)
        for i in range(len(inner.vertices)):
